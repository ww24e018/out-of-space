...
- **Colour scheme:** Extension-based category mapping (~8 buckets: code, images, documents, archives, media, config, data, other). Directories use depth-based muted shades. Palette definitions live in `palettes.ts`; `colorScale.ts` accepts a `ColorPalette` parameter and is reusable across visualisation modes. Users can switch palettes via a dropdown in the header (Solarized default, Legacy, and two HSL Radial variants with evenly-spaced hues); state is managed by the `palette` Pinia store.
